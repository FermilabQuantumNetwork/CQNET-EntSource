...
START_TIME = '2019-11-27 17:15:03'
END_TIME = '2019-11-28 01:40:00'

#START_TIME = '2019-12-11 17:19:34'
#END_TIME = '2019-12-11 20:29:59'
...
try:
	...
	fig, axs = plt.subplots(6,1, num=239, sharex = True)
	xmin=178#13
	xmax=189#25
	GUI_indices = np.where((time_GUI_el_mins > xmin) & (time_GUI_el_mins < xmax))
	GUI_indices = GUI_indices[0]
	#print(GUI_indices)
	T_indices = np.where((time_T1_el_mins > xmin) &(time_T1_el_mins < xmax))
	T_indices = T_indices[0]
	#print(T_indices)
	P_indices = np.where((time_T1_el_mins > xmin) &(time_T1_el_mins < xmax))
	P_indices = P_indices[0]
	#print(P_indices)

	#minVis = np.mean(and2[GUI_indices[0]:GUI_indices[-1]])
	#minVisStd=np.std(and2[GUI_indices[0]:GUI_indices[-1]])
	minVis = np.sum(and2[GUI_indices[0]:GUI_indices[-1]])
	print(minVis)
	minVisStd=np.sqrt(minVis)
	...
	#and1
	axs[3].plot(time_GUI_el_mins[GUI_indices[0]:GUI_indices[-1]], and1[GUI_indices[0]:GUI_indices[-1]],  linestyle = 'none', marker = '.', markersize = 8)
	axs[3].set_ylabel("and1")
	#axs[3].set_ylim(0,25)
	axs[3].grid()

	#and2
	axs[4].plot(time_GUI_el_mins[GUI_indices[0]:GUI_indices[-1]], and2[GUI_indices[0]:GUI_indices[-1]],  linestyle = 'none', marker = '.', markersize = 8, label = "Mean = "+"{:.3f}".format(minVis)+"\nStd = "+"{:.3f}".format(minVisStd))
	...
	#and3
	axs[5].plot(time_GUI_el_mins[GUI_indices[0]:GUI_indices[-1]], and3[GUI_indices[0]:GUI_indices[-1]],  linestyle = 'none', marker = '.', markersize = 8)#, label = "Mean = "+"{:.3f}".format(mean(and3[GUI_indices[0]:GUI_indices[-1]]))+"\nStd = "+"{:.3f}".format(np.std(and3[GUI_indices[0]:GUI_indices[-1]])))
	...
	#plt.legend()
	fig.suptitle("and2 Nmin = "+"{:.3f}".format(minVis)+", and2 sqrt(Nmin) = "+"{:.3f}".format(minVisStd))
	...
	fig, axs = plt.subplots(6,1, num=240, sharex = True)
	xmin=98#62
	xmax=109#74
	GUI_indices = np.where((time_GUI_el_mins > xmin) & (time_GUI_el_mins < xmax))
	GUI_indices = GUI_indices[0]
	...
	#maxVis = np.mean(and2[GUI_indices[0]:GUI_indices[-1]])
	#maxVisStd=np.std(and2[GUI_indices[0]:GUI_indices[-1]])

	maxVis = np.sum(and2[GUI_indices[0]:GUI_indices[-1]])
	print(maxVis)
	maxVisStd=np.sqrt(maxVis)
	...
	axs[5].set_xlim(xmin, xmax)
	#plt.legend()
	fig.suptitle("and2 Nmax = "+"{:.3f}".format(maxVis)+", and2 sqrt(Nmax) = "+"{:.3f}".format(maxVisStd))
	plt.xlabel('Elapsed time (min)', fontsize =16)
	#plt.savefig(figname)
	...
	#and2
	axs[4].plot(time_GUI_el_mins[GUI_indices[0]:GUI_indices[-1]], and2[GUI_indices[0]:GUI_indices[-1]],  linestyle = 'none', marker = '.', markersize = 8, label = "Min Counts = {}".format(min(and2[GUI_indices[0]:GUI_indices[-1]]))+" , Max Counts = {}".format(max(and2[GUI_indices[0]:GUI_indices[-1]])))
	axs[4].set_ylabel("and2")
	#axs[4].legend()
	#axs[4].set_ylim(0,60)
	axs[4].grid()
	...
	#and3
	axs[5].plot(time_GUI_el_mins[GUI_indices[0]:GUI_indices[-1]], and3[GUI_indices[0]:GUI_indices[-1]],  linestyle = 'none', marker = '.', markersize = 8)
	axs[5].set_ylabel("and3")
	#axs[5].set_ylim(0,25)
	axs[5].grid()
