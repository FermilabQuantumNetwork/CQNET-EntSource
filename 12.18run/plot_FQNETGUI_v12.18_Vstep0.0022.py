...
START_TIME = '2019-12-18 13:36:38'
END_TIME = '2019-12-18 17:00:00'
...
try:
	...
	fig, axs = plt.subplots(8,1, num=248, sharex = True)
	xmin=58
	xmax=69
	tab2_indices = np.where((time_tab2_el_mins > xmin)&(time_tab2_el_mins < xmax))
	tab2_indices = tab2_indices[0]

	T_indices = np.where((time_T1_el_mins > xmin) &(time_T1_el_mins < xmax))
	T_indices = T_indices[0]

	P_indices = np.where((time_P_el_mins > xmin) &(time_P_el_mins < xmax))
	P_indices = P_indices[0]

	histo_indices = np.where((time_histo_el_mins > xmin)&(time_histo_el_mins < xmax))
	histo_indices = histo_indices[0]

	minVis = np.sum(and2[tab2_indices[0]:tab2_indices[-1]])
	print(minVis)
	...
	fig, axs = plt.subplots(8,1, num=258, sharex = True)
	xmin=189
	xmax=200
	tab2_indices = np.where((time_tab2_el_mins > xmin)&(time_tab2_el_mins < xmax))
	tab2_indices = tab2_indices[0]
	...
	maxVis = np.sum(and2[tab2_indices[0]:tab2_indices[-1]])
	print(maxVis)
	...
	#and2
	axs[4].plot(time_tab2_el_mins[tab2_indices[0]:tab2_indices[-1]], and2[tab2_indices[0]:tab2_indices[-1]],  linestyle = 'none', marker = '.', markersize = 8, label = "Max Counts = {}".format(max(and2[tab2_indices[0]:tab2_indices[-1]]))+" , Max Counts = {}".format(max(and2[tab2_indices[0]:tab2_indices[-1]])))
	axs[4].set_ylabel("and2")
	#axs[4].legend()
	#axs[4].set_ylim(0,60)
	axs[4].grid()
